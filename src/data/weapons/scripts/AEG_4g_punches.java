...
public class AEG_4g_punches implements EveryFrameWeaponEffectPlugin, OnFireEffectPlugin, OnHitEffectPlugin {
    ...
    private float overlap = 0, heat = 0, originalRArmPos = 0f, originalLArmPos = 0f;
    ...
    private boolean rightPunchActive = false;
    private boolean leftPunchActive = false;
    private boolean willKnifeActive = false;
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (weapon.getSlot().getId().equals("WS0007")) { // Right punch
            if (!leftPunchActive && !willKnifeActive) {
                rightPunchActive = true;
                weapon.getSprite().setCenterY(originalRArmPos - (8 * sineA) + (8 * sinceG));
            }
        } else if (weapon.getSlot().getId().equals("WS0006")) { // Left punch
            if (!rightPunchActive && !willKnifeActive) {
                leftPunchActive = true;
                weapon.getSprite().setCenterY(originalLArmPos - (8 * sineA) + (8 * sinceG));
            }
        } else if (weapon.getSlot().getId().equals("WS0008")) { // Will knife
            if (!rightPunchActive && !leftPunchActive) {
                willKnifeActive = true;
                weapon.getSprite().setCenterY(originalRArmPos - (8 * sineA) + (8 * sinceG));
            }
        }
        ...
        // Reset active states after punch completes
        if (weapon.getChargeLevel() <= 0) {
            rightPunchActive = false;
            leftPunchActive = false;
            willKnifeActive = false;
        }
    }
