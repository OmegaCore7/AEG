...
public class AEG_BlackHoleGeneratorPlugin extends BaseEveryFrameCombatPlugin
{
    ...
    private final Map<DamagingProjectileAPI, Float> projs = new HashMap<>();
    // Tracks holes - key = location, value = time to expire
    private final Map<Vector2f, Float> holes = new HashMap<>();
    ...
    {
        for (Map.Entry<DamagingProjectileAPI, Float> tmp : projs.entrySet())
        {
            float remaining = tmp.getValue();
            remaining -= curTime;
    ...
    private void updateHoles(float time)
    {
        ...
        for (Iterator<Map.Entry<Vector2f, Float>> iter = holes.entrySet().iterator(); iter.hasNext(); )
        {
            ...
            Vector2f center = tmp.getKey();
            for (CombatEntityAPI victim : CombatUtils.getEntitiesWithinRange(
                    center, GRAVITY_WELL_RADIUS))
            {
                ...
                if (numUpdates % FRAMES_PER_DAMAGE_POINT == 0)
                {
                    ...
                    if (((victim instanceof ShipAPI) || (victim instanceof MissileAPI))
                            && MathUtils.isWithinRange(victim, center, DAMAGE_ZONE_RADIUS))
                    {
    ...
    private void checkProjs()
    {
        ...
        // TODO: Replace this with separate on-hit effect
        if (!projs.isEmpty())
        {
